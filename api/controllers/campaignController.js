          ...
          conn.query(insertVoters, [voterValues], function (error3, result2) {
            ...
            conn.query(
              ...
              function (error4, result3) {
                ...
                const insertBallotChoices = `INSERT INTO choices (campaign_id, question_id, name, title, bio, image_filepath, vote_count, choice_placement) VALUES ?`;
...
exports.submit_paper_ballot = (req, res) => {
  ...
  conn.beginTransaction((err) => {
    ...
    const ballotValues = ballotIds.map((b) => [
      b,
      campaign_id,
      new Date(),
      "PAPER",
    ...
    conn.query(insertBallot, [ballotValues], function (ballotError, ballots) {
      ...
      conn.query(
        insertSelections,
        [ballotSelections],
        function (selectionsError, response) {
          if (selectionsError) {
            return conn.rollback(() => {
          ...
          const countValues = selections.map((selection) =>
            Number(selection.response_id)
          );
      ...
      conn.query(insertBallot, ballotValues, function (error1, result1) {
        ...
        conn.query(
          ...
          function (error2, result2) {
            ...
            const countValues = selections.map(
              (selection) => selection.response_id
            );
  ...
  conn.beginTransaction((err) => {
    ...
    conn.query(updateCampaign, campaignValues, function (error1, result1) {
      ...
      conn.query(updateQuestions, [questionValues], function (error2, result2) {
        ...
        const updateChoices = `INSERT INTO choices (response_id, campaign_id, question_id, name, title, bio, image_filepath, choice_placement) VALUES ?
        ON DUPLICATE KEY UPDATE name=VALUES(name), title=VALUES(title), bio=VALUES(bio), image_filepath=VALUES(image_filepath), choice_placement=VALUES(choice_placement)`;
